#### Function to predict from CSP parameters

get_preds_CSP <- function(actual_days, bio_data, climate_data, var="temp", tot_years, pred_years,
                          date_var = c("DOY95", "lay_mean")){
  # actual days will be entered as length = 2 (a vector of start and end)
  ...
  marker <- which(tot_years == min(unique(bio_data$Year)))
  marker1 <- which(tot_years == max(unique(bio_data$Year)))
